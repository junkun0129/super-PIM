...
import { getMainAsbIdApi } from "../../api/asset.api";
...
const SeriesListPage = () => {
  ...
  const getSeries = async () => {
    const { data, total, result , imgs} = await getProductListApi({
      is: "1",
      pg: currentPage,
      ps: pagination,
      ws: workspace,
    ...
    const res = await getMainAsbIdApi()
    ...
    const newDataSource: Row[] = data.map((pr, i) => ({
      ...
      hinban: pr.pr_hinban,
      name: pr.pr_name,
      img:<img width={50} height={50} src={`${import.meta.env.VITE_BASE_URL}/${imgs[pr.pr_cd]}`}/>,
